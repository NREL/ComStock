...
module OsLib_Reporting
  ...
  def self.interior_lighting_section(model, sqlFile, runner, name_only = false)
    ...
    rows.each do |row|
      ...
      columns_query.each do |header|
        ...
        if not table[:source_units][column_counter] == ''
          ...
          sum_total_power += row_data_ip if header == 'Total Power'
          sum_annual_consumption += row_data_ip if header == 'Consumption'
        else
    ...
    lighting_full_load_hrs = sum_annual_consumption > 0 ? sum_total_power / sum_annual_consumption : 0
    runner.registerValue('lighting_full_load_hrs', lighting_full_load_hrs, 'hrs')
