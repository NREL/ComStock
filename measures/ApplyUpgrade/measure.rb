...
require 'csv'
require 'openstudio'
File.absolute_path(File.join(__dir__, '..', '..', 'lib', 'resources'))
...
class ApplyUpgrade < OpenStudio::Ruleset::ModelUserScript
  ...
  def num_options
    200 # Synced with SimulationOutputReport measure
  end

  def num_costs_per_option
    2 # Synced with SimulationOutputReport measure
  end
  ...
  def arguments(_model)
    ...
    for option_num in 1..num_options
      ...
      for cost_num in 1..num_costs_per_option
        ...
        cost_value = OpenStudio::Ruleset::OSArgument.makeDoubleArgument("option_#{option_num}_cost_#{cost_num}_value",
                                                                        false)
        ...
        cost_multiplier = OpenStudio::Ruleset::OSArgument.makeChoiceArgument(
          "option_#{option_num}_cost_#{cost_num}_multiplier", choices, false
        )
  ...
  def run(model, runner, user_arguments)
    ...
    for option_num in 1..num_options
      if option_num == 1
        ...
        next  unless arg.is_initialized

        arg = arg.get
      end
      next if arg.strip.empty?
    ...
    for option_num in 1..num_options
      ...
      unless options.keys.include?(option_num)
    ...
    workflow_json = File.join(resources_dir, workflow_json) unless workflow_json.nil?

    # Process package apply logic if provided
    apply_package_upgrade = true
    unless package_apply_logic.nil?
      # Apply this package?
      apply_package_upgrade = evaluate_logic(package_apply_logic, runner)
      return false if apply_package_upgrade.nil?
    end
    ...
    if apply_package_upgrade
      ...
      options.each do |option_num, option|
        ...
        if options_apply_logic.include?(option_num)
          ...
          return false if apply_option_upgrade.nil?
        end

        unless apply_option_upgrade
        ...
        for cost_num in 1..num_costs_per_option
          cost_value = runner.getOptionalDoubleArgumentValue("option_#{option_num}_cost_#{cost_num}_value",
                                                             user_arguments)
          cost_value = 0.0 if cost_value.nil?
      ...
      measures.keys.each do |measure_subdir|
        parameters.each do |parameter_name|
          ...
          options_measure_args = get_measure_args_from_option_names(lookup_file, [existing_option_name],
                                                                    parameter_name, runner)
          options_measure_args[existing_option_name].each do |measure_subdir2, args_hash|
            ...
            args_hash.each do |k, v|
              next if measures[measure_subdir][0].key?(k)
      ...
      measures_result = apply_measures(measures_dirs, measures, runner, model, workflow_json, 'measures-upgrade.osw',
                                       true)
      if !measures_result
