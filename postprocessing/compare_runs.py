...
def main():
    ...
    comstock_a = cspp.ComStock(
        s3_base_dir= '',#'C:/Users/aallen/Documents/ComStock/unocc_oa_control/baseline_parquet',  # If run not on S3, download results_up**.parquet manually
        comstock_run_name='',  # Name of the run on S3
        ...
        include_upgrades=True  # False if not looking at upgrades ##AA changed to true 
        )

    # Second ComStock run
    comstock_b = cspp.ComStock(
        s3_base_dir='C:/Users/aallen/Documents/ComStock/unocc_oa_control/up_01_parquet',  # If run not on S3, download results_up**.parquet manually
        comstock_run_name='',  # Name of the run on S3
        ...
        skip_missing_columns=False,  # False if you want to ensure you have all data specified for export
        reload_from_csv=False,  # True if CSV already made and want faster reload times
        include_upgrades=True  # False if not looking at upgrades ##AA changed to true 
        )
