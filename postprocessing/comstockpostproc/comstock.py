...
class ComStock(NamingMixin, UnitsMixin, GasCorrectionModelMixin, S3UtilitiesMixin):
    ...
    def add_national_scaling_weights(self, cbecs: CBECS, remove_non_comstock_bldg_types_from_cbecs: bool, use_hard_coded_scaling_weights: bool, scaling_factors: dict):

        # Remove CBECS entries for building types not included in the ComStock run
        if use_hard_coded_scaling_weights:
            ...
            self.building_type_weights = scaling_factors
            self.data = self.data.with_columns((pl.col(self.BLDG_TYPE).cast(pl.Utf8).replace(scaling_factors, default=None)).alias(self.BLDG_WEIGHT))
