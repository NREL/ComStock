...
def run_measure(model, measure, argument_map, runner)
    begin
      ...
      # log all the measure arguments
      measure_name = measure.name.to_s.downcase.gsub(' ', '_')
      argument_map.each do |arg_name, arg|
      ...
      runner.registerError("Measure Failed with error #{e.message} in: #{e.backtrace.join("\n")}")
      return false
    end
    return true
end
