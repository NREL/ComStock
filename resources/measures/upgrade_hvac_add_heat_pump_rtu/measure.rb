...
class AddHeatPumpRtu < OpenStudio::Measure::ModelMeasure
  ...
  def arguments(_model)
    ...
    setback_value.setDisplayName('Amount by which temperatures are set back during unoccupied periods in heating mode')
  ...
  def get_8760_values_from_schedule_ruleset(model, schedule_ruleset) #AA added temporarially, until merged into standards 
  ...
  def adjust_setpoint(value, index, occ_schedule, setback_value, tstat_max)
      new_value = value
      if  occ_schedule[index] == 0 
	      new_value = tstat_max - setback_value
	  end 
  ...
  def run(model, runner, user_arguments)
    ...
    selected_air_loops.sort.each do |air_loop_hvac|
	  ...
	  zones.sort.each do |thermal_zone|
		  ...
		  if tstat_profile_size == 1  #handle case without setbacks currently, screen for space types with 24/7 ops, deal with holidays
